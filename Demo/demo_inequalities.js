...
function fill(row, col) {
    ...
    if(row == 4 && col == 3) {
        ...
        coeff = [[1, 3], [2, -4], [1, 0]];
        sign = ["lessEqual", "greatEqual", "lessEqual"];
    }
    else if(row == 4 && col == 2) {
        coeff = [[2, -4, 3], [-1, 1, 4], [1, 0, 9], [1, 1, 3]];
    ...
    document.getElementById("ineq_matrix").innerHTML = "<hr> Example system matrix entries:";
    ...
    btn.onclick = function () {analyse(row, col)};
...
function evaluateNext(orderNonEl, orderEl, matrix, signs, arrayOfMatrices, arrayOfSigns, callback) {
    if(orderNonEl.length > 0) {
        ...
        if(range[1] == "less" ) {
            rightStr += "\x3C" + " " + range[0];
        }
        else if(range[1] == "lessEqual") {
            rightStr += "\u2264" + " " + range[0];
        }
        else
            rightStr += "\x3C &infin;"; // less
        if(range[3] == "great") {
            leftStr += range[2] + " " + "\x3C"; // less
        }
        else if(range[3] == "greatEqual") {
            leftStr += range[2] + " " + "\u2264"; // lessEqual
        }
        else
            leftStr += "-&infin; \x3C "; // less
